...
def update_typeX(coll,doc,bulkopt,request):
    '''
    request.append(
            UpdateOne( {"tag_group_nm": doc["group_nm"], "tag_nm":doc["tag_nm"],"date": doc["ts"].strftime("%Y-%m-%d %H")},
                     {
                         "$min" : {"first": doc["ts"]},
                         "$max" : {"last": doc["ts"]},
                         "$inc" : {"nsamples" : 1},
...
def worker(uri,dbname,colname,group):
    ...
    start_time=datetime.datetime(2020,5,31,0,0,0)
    # 6*60*24 : 1day
    # 6*60*24*30 : 1Month
